...
def move_json_files(source_dir, destination_dir):
    """
    Recursively moves all .json files from a source directory to a destination directory.
...
def main(argv=None):
    ...
    try:
        ...
        allPlayers = requests.get('https://api-fantasy.llt-services.com/api/v4/players?x-lang=es')
        if allPlayers.status_code == 200:
            allPayload = allPlayers.json()
            for index in range(0, len(allPayload)):
                response = requests.get('%s/%s' % (players_endpoint, allPayload[index]['id']))
                if response.status_code == 200:
                    payload = response.json()
                    team_id = payload['team']['id']
                    team_shortname = payload['team']['name']
                    team_folder = 'laliga/%s-%s' % (team_id, team_shortname)
                    if not os.path.exists(team_folder):
                        os.makedirs(team_folder)
                    player_id = payload['id']
                    player_slug = payload['nickname']
                    ...
                    if player_status != 'out_of_league':
                        ...
                        f.close()                
                        logger_.info('Scraped player with id=%s' % player_id)
                        ...
                        player_csv_row += ',"%s","%s",%s,%s' % (payload['name'],payload['nickname'],'',payload['positionId'])
                        player_csv_row += ',%s,%s,%s' % (positions[payload['positionId'] - 1],payload['marketValue'],payload['playerStatus'])
                        for w in range(0,38):
                        ...
                        logger_.warning('Player with id=%s not in LaLiga anymore' % allPayload[index])
        ...
        move_json_files("laliga", "laliga/json")
